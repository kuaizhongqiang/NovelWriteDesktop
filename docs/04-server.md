...
> 对应前端 `web/src/utils/commandBus.ts` 的 Command 抽象层。

#### AI Agent 操作权限边界

AI Agent 的 Tool 操作按危险程度分为三级，前端通过弹窗确认：

| 等级 | 包含操作 | 确认要求 |
|------|----------|----------|
| 🔵 **读 (read)** | `get_novel`, `get_roles`, `get_outline`, `get_chapter`, `get_writing_style` | 自动执行，无需确认 |
| 🟡 **写 (write)** | `update_novel_base`, `add_role`, `update_role`, `add_phase`, `update_phase`, `add_chapter_outline`, `update_chapter_outline`, `patch_chapter_content`, `replace_chapter_content`, `update_writing_style` | 用户确认弹窗 |
| 🔴 **删 (delete)** | `delete_role`, `delete_phase`, `delete_chapter_outline` | 二次确认弹窗 |

#### 回滚机制

每次 Agent 发起的变更操作遵循以下流程：

```
Agent 请求变更
    → 命令进入 pending_changes 队列
    → 前端弹窗确认（写/删操作）
    → 用户确认 → 执行并记录 audit log
    → 用户拒绝 → 丢弃，返回错误给 Agent
```

**Audit Log** 记录在内存中（`commandBus.ts` 的 `auditLog`），包含：
- 时间戳
- 命令描述
- 操作层级（read/write/delete）
- 执行结果（成功/失败/被拒）

同一 Session 内的操作可通过 `GET /api/ai/session/:id/stats` 获取操作历史。
